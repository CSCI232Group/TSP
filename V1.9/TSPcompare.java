...
public class TSPcompare                                                 //Traveling Sales Problem Comparing class
{
    ...
    public void twiceAroundTree()
    {
        ...
        ArrayList<Kruskal> trees=new ArrayList<Kruskal>();
        trees.add(new Kruskal());
        
        
        trees.get(0).addEdge(edgeWeights.get(0));
       // trees.get(0).addEdge(edgeWeights.get(1));
        int contains;
        boolean bad=false;
        ...
        for (int i=1;i<edgeWeights.size();i++){
        	...
        	for(int j=0;j<trees.size();j++){
        		...
        		contains=trees.get(j).contains(edgeWeights.get(i));
        		if(contains==0){
        			bad=true;
        		}
        		else if(contains==-1){
        			for(int k=0;k<trees.size();k++){
        				if(trees.get(k).getCities().contains(edgeWeights.get(i).to)&&k!=j){
        					trees.get(j).addEdge(edgeWeights.get(i));
        					trees.get(j).join(trees.get(k));
        					trees.remove(k);
        					break innerloop;
        				}
        			...
        			for(int k=0;k<trees.size();k++){
        				if(trees.get(k).getCities().contains(edgeWeights.get(i).from)&&k!=j){
        					trees.get(j).addEdge(edgeWeights.get(i));
        					trees.get(j).join(trees.get(k));
        					trees.remove(k);
        					break innerloop;
        		...
        		if(j==trees.size()&&!bad){
        			trees.add(new Kruskal());
        			trees.get(trees.size()-1).addEdge(edgeWeights.get(i));
        		}
