...
public class TSPcompare
{
    ...
    public void twiceAroundTree()
    {
        ...
        int edges = 0;
        while(edges != edgeWeights.size()-1)
        {
            ...
            for(int i = 0; i < edgeWeights.size(); i++)
            {
                ...
                int[] toNeighbors = new int[edgeWeights.size()];
                int[] fromNeighbors = new int[edgeWeights.size()];
                int toPos, fromPos;
                toPos = fromPos = 0;
                ...
                {
                    System.out.printf("Started MST loop\n");
                    check = mst.get(j);
                    check_a = check.getTo();
                    ...
                    if(to == check_a)
                    {
                        toNeighbors[toPos] = check_b.ID;
                        toPos++;
                    }
                    else if(to == check_b)
                    {
                        toNeighbors[toPos] = check_a.ID;
                        toPos++;
                    }
                    if(from == check_a)
                    {
                        fromNeighbors[fromPos] = check_b.ID;
                        fromPos++;
                    }        
                    else if(from == check_b)
                    {
                        fromNeighbors[fromPos] = check_a.ID;
                        fromPos++;
                    }                 
                }                                                
                for(int l = 0; l < toNeighbors.length; l++)
                {
                    for(int m = 0; m < fromNeighbors.length; m++)
                    {
                        if(toNeighbors[l] == 0 && fromNeighbors[m] == 0)
                        {m = fromNeighbors.length;
                        l = toNeighbors.length;}
                        
                        else if(toNeighbors[l] == fromNeighbors[m])
                        {
                            makesLoop = true;
                ...
                //If does not make loop
                if(!makesLoop)
                    {System.out.printf("Added edge\n");
                    mst.add(current);
                    edges++;}                                
                ...
                //Search list of added cities for ones that contain either city in Current
                //Debugging
                //System.out.printf("%f\n", current.getWeight());
        ...
        for(EdgeWeight e : mst)
        {
            System.out.printf("%d - %d : %f\n", e.getTo().ID, e.getFrom().ID, e.getWeight());
        }
